...
1. [⚠️ Первый запуск CI — обязательный шаг одобрения](#️-первый-запуск-ci--обязательный-шаг-одобрения)
2. [Способ 1 — Скачать готовый файл (просто и быстро)](#способ-1--скачать-готовый-файл-просто-и-быстро)
3. [Способ 2 — Собрать из исходного кода](#способ-2--собрать-из-исходного-кода)
4. [Открытие в Roblox Studio](#открытие-в-roblox-studio)
5. [Тестирование игры](#тестирование-игры)
6. [Публикация на Roblox](#публикация-на-roblox)
7. [Как играть — краткое руководство](#как-играть--краткое-руководство)
8. [Структура проекта](#структура-проекта)

---

## ⚠️ Первый запуск CI — обязательный шаг одобрения

> **Видишь статус «Action required» (оранжевый значок ⚠️) в разделе Actions?**
> Это нормально. GitHub требует однократного одобрения от владельца репозитория при первом запуске нового воркфлоу.

### Почему так происходит?

GitHub по умолчанию **блокирует** первый запуск любого нового workflow-файла (`.github/workflows/build.yml`), добавленного через Pull Request от стороннего участника (в том числе от Copilot-бота). Это защита от запуска вредоносного кода в CI. Ни одна строчка билда **не выполнялась** — нужно твоё разрешение.

### Как одобрить и запустить сборку?

**Шаг 1.** Перейди в репозиторий на GitHub → вкладка **Actions**.

**Шаг 2.** Нажми на запуск со статусом **"Action required"** (⚠️ оранжевый).

**Шаг 3.** На странице запуска появится жёлтый баннер с текстом:
> *"This workflow requires approval from the repository owner"*

**Шаг 4.** Нажми кнопку **"Approve and run"** (Одобрить и запустить).

**Шаг 5.** Воркфлоу запустится. Через ~2 минуты появится зелёная галочка ✅ и артефакт **`EvergreenCounty-rbxlx`** будет готов к скачиванию.

> 💡 **Важно:** Это нужно сделать **только один раз**. После слияния PR в `main` все последующие сборки будут запускаться автоматически без одобрения.

### Альтернатива: запустить вручную

Если кнопки «Approve and run» нет, используй ручной запуск:
1. Actions → **"Build EvergreenCounty.rbxlx"** → кнопка **"Run workflow"** → выбери ветку → **"Run workflow"**.
...
> ⚠️ Сначала выполни шаг одобрения выше — иначе файла ещё нет.
