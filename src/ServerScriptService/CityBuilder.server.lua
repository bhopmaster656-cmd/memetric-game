...
-- ── Ensure Baseplate exists ────────────────────────────────────────────────────
-- Embedded in the RBXLX as static geometry, but always recreated dynamically
-- here as a safety net in case the RBXLX version was not loaded correctly.
-- This MUST happen before CharacterAutoLoads is re-enabled so the ground is
-- always present when the first character spawns.
...
-- ── Ensure SpawnLocation exists ────────────────────────────────────────────────
-- Same pattern: prefer the static RBXLX version, fall back to a dynamic one.
...
-- ── Build the city inside pcall so any error never prevents characters loading ─
local _buildOk, _buildErr = pcall(function()
...
if _buildOk then
    ...
    warn("[CityBuilder] City build error (ground is still solid):", tostring(_buildErr))
end
